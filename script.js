...
function renderTimeSelectors(){
  ...
  $('timeSelectors').innerHTML=defs.map(([key,title,isNew])=>timeCardHtml(key,title,isNew)).join('');
  ['sameTimeNonWinter','sameTimeWinter'].forEach(id=>$(id)?.addEventListener('change', syncSameTimes));
  syncSameTimes();
}
function timeCardHtml(key,title,isNew){return `<div class="time-card"><h4>${title}</h4>${isNew?`<label class="checkline"><input id="${key==='newNonWinter'?'sameTimeNonWinter':'sameTimeWinter'}" type="checkbox" checked /> 기존과 동일</label>`:''}<div id="body_${key}"><div class="time-tools"><button type="button" class="mini" onclick="selectAllHours('${key}')">24시간</button><button type="button" class="mini" onclick="clearHours('${key}')">선택 초기화</button></div><div class="time-grid" id="grid_${key}">${Array.from({length:24},(_,h)=>`<button type="button" class="hour-btn" data-key="${key}" data-hour="${h}" onclick="toggleHour('${key}',${h})">${String(h).padStart(2,'0')}:00~${String((h+1)%24).padStart(2,'0')}:00</button>`).join('')}</div><p class="small">미선택 시 24시간 가동으로 계산</p></div></div>`}
...
function selectedHoursRaw(key){return gridBtns(key).filter(b=>b.classList.contains('on')).map(b=>Number(b.dataset.hour)).sort((a,b)=>a-b)}
...
function syncSameTimes(){
  if($('sameTimeNonWinter')?.checked){setHoursSilent('newNonWinter',selectedHoursRaw('oldNonWinter')); $('body_newNonWinter').classList.add('hidden')} else {$('body_newNonWinter').classList.remove('hidden')}
  if($('sameTimeWinter')?.checked){setHoursSilent('newWinter',selectedHoursRaw('oldWinter')); $('body_newWinter').classList.add('hidden')} else {$('body_newWinter').classList.remove('hidden')}
}
...
function clearEquipmentInput(){ $('eqName').value=''; $('oldKw').value=''; $('newKw').value=''; $('oldCount').value='1'; $('newCount').value='1'; $('oldRunMin').value='60'; $('newRunMin').value='60'; $('sameKw').checked=true; $('sameCount').checked=true; $('sameRunMin').checked=true; $('sameTimeNonWinter').checked=true; $('sameTimeWinter').checked=true; renderTimeSelectors(); syncSameInputs(); }
...
function selectedComparable(arr){return (arr && arr.length ? arr : Array.from({length:24},(_,i)=>i)).join(',')}
...
function renderSavingReport(){
  try{
    ...
    $('savingResult').innerHTML=`<div class="card" id="savingReport"><h3>전력절감 검토 결과</h3><div class="actions report-actions"><button class="secondary" onclick="copyElementText('savingReport')">전체 결과 복사</button><button class="secondary" onclick="printSavingReport()">PDF로 열기/저장</button><button class="secondary" onclick="exportSavingExcel()">엑셀로 저장</button></div><div class="basis">계약종별: ${esc(tariff.label)} · 산정기간: ${dates[0].toISOString().slice(0,10)} ~ ${dates[dates.length-1].toISOString().slice(0,10)} · 산정일수: ${dates.length}일</div>${rateTable(tariff)}<div class="kpi"><div class="box"><div class="k">기존 사용량</div><div class="v">${num(total.oldKwh,0)}kWh</div></div><div class="box"><div class="k">변경 사용량</div><div class="v">${num(total.newKwh,0)}kWh</div></div><div class="box"><div class="k">절감량</div><div class="v">${num(total.saveKwh,0)}kWh</div></div></div>${installTable(results)}${operationTable(results)}${effectTable(results,total)}${basisDetails(results,dates,seasonDays)}</div>`;
    $('savingResult').classList.remove('hidden');
  }catch(e){alert(e.message)}
}
function conditionText(kw,count){return `${num(kw,2)}kW × ${count}대 = ${num(kw*count,2)}kW`}
function installTable(results){return `<h4>1. 설치현황 <button class="mini" onclick="copySection(this)">복사</button></h4><div class="table-wrap"><table class="report-table"><thead><tr><th>No</th><th>설비명</th><th>기존 조건</th><th>변경 조건</th><th>개선 방식</th></tr></thead><tbody>${results.map((x,i)=>{const same=!powerChanged(x.item); const oldTxt=conditionText(x.item.oldKw,x.item.oldCount); const newTxt=same?'기존과 동일':conditionText(x.item.newKw,x.item.newCount); return `<tr><td class="center">${i+1}</td><td>${esc(x.item.name)}</td><td class="right">${oldTxt}</td><td class="right">${newTxt}</td><td>${classifyItem(x.item)}</td></tr>`}).join('')}</tbody></table></div>`}
...
function operationTable(results){const rows=[]; results.forEach((x,i)=>{if(!hoursChanged(x.item)) return; const row1=`<tr><td class="center" rowspan="2">${i+1}</td><td rowspan="2">${esc(x.item.name)}</td><td>봄·여름·가을(3~10월)</td><td>${operationText(x.item,'nonWinter',false,'springAutumn')}</td><td>${operationText(x.item,'nonWinter',true,'springAutumn')}</td><td class="right">${num(actualRunHours(x.item,'nonWinter',false)-actualRunHours(x.item,'nonWinter',true),2)}h/일</td></tr>`; const row2=`<tr><td>겨울철(11~2월)</td><td>${operationText(x.item,'winter',false,'winter')}</td><td>${operationText(x.item,'winter',true,'winter')}</td><td class="right">${num(actualRunHours(x.item,'winter',false)-actualRunHours(x.item,'winter',true),2)}h/일</td></tr>`; rows.push(row1+row2);}); if(!rows.length) return ''; return `<h4>2. 운전시간 변경 조건 <button class="mini" onclick="copySection(this)">복사</button></h4><div class="table-wrap"><table class="report-table"><thead><tr><th>No</th><th>설비명</th><th>구분</th><th>기존 조건</th><th>변경 조건</th><th>가동시간 감소</th></tr></thead><tbody>${rows.join('')}</tbody></table></div>`}
function effectTable(results,total){return `<h4>3. 절감효과 <button class="mini" onclick="copySection(this)">복사</button></h4><div class="table-wrap"><table class="report-table"><thead><tr><th>설비명</th><th>기존 사용량</th><th>변경 사용량</th><th>절감량</th><th>절감률</th><th>절감금액</th></tr></thead><tbody>${results.map(x=>`<tr><td>${esc(x.item.name)}</td><td class="right">${num(x.calc.oldKwh,0)}kWh</td><td class="right">${num(x.calc.newKwh,0)}kWh</td><td class="right bold">${num(x.calc.saveKwh,0)}kWh</td><td class="right">${num(x.calc.saveRate,1)}%</td><td class="right bold">${won(x.calc.saveMoney)}</td></tr>`).join('')}<tr><th>합계</th><th class="right">${num(total.oldKwh,0)}kWh</th><th class="right">${num(total.newKwh,0)}kWh</th><th class="right">${num(total.saveKwh,0)}kWh</th><th class="right">${num(total.saveRate,1)}%</th><th class="right">${won(total.saveMoney)}</th></tr></tbody></table></div>`}
function rateTable(tariff){const rows=['summer','springAutumn','winter'].map(s=>{if(tariff.type==='tou'){const e=tariff.energy[s]; return `<tr><td>${seasonsKo[s]}</td><td class="right">${e.light}</td><td class="right">${e.mid}</td><td class="right">${e.peak}</td></tr>`} return `<tr><td>${seasonsKo[s]}</td><td colspan="3" class="right">${tariff.energy[s]}</td></tr>`}).join(''); return `<h4>적용 전력량 요금 단가 <button class="mini" onclick="copySection(this)">복사</button></h4><div class="table-wrap"><table class="report-table"><thead><tr><th>계절</th><th>경부하</th><th>중간부하</th><th>최대부하</th></tr></thead><tbody>${rows}</tbody></table></div>`}
function basisDetails(results,dates,seasonDays){return `<details><summary>계산근거 보기</summary><div class="basis">운영기간 일수: 여름 ${seasonDays.summer}일, 봄·가을 ${seasonDays.springAutumn}일, 겨울 ${seasonDays.winter}일<br>사용량 = 부하(kW) × 대수 × 선택 가동시간(h) × 가동분/60<br>절감량 = 기존 사용량 - 변경 사용량<br>절감금액 = 시간대별 절감전력량 × 한전 전력량요금 단가</div></details>`}
